...
function sendUDPCommand(command) {
    ...
    try {
        // Perubahan di sini: pastikan command dikonversi ke string dan hasil enkripsi adalah Buffer
        const commandStr = String(command); // Pastikan command adalah string
        const encryptedMessage = enkripsi(commandStr); // Enkripsi pesan
        
        // Pastikan hasil enkripsi adalah Buffer atau string yang valid untuk UDP
        const messageBuffer = Buffer.from(encryptedMessage); // Konversi ke Buffer jika belum
...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (message) => {
        if (clientType === 'web') {
            try {
                // Dekripsi pesan dari web client
                let parsedMessage;
                try {
                    // Coba dekripsi jika pesan terenkripsi
                    const decryptedMessage = dekripsi(message.toString());
                    parsedMessage = JSON.parse(decryptedMessage);
                } catch {
                    // Jika gagal, coba parse langsung (untuk kompatibilitas)
                    parsedMessage = JSON.parse(message);
                }
                
                if (!isNaN(parsedMessage)) {
                    console.log(`Received setpoint: ${parsedMessage}°`);
                    PID.target_angle = parsedMessage * Math.PI / 180;
            ...
            if (!deviceState.authenticated) {
                try {
                    // Parsing auth data - simplifikasi dengan JSON.parse langsung
                    ...
                    try {
                        ...
                        console.error("JSON parse failed, trying to decode:", e);
                        try {
                            // Try to decode without JSON parsing
                            authData = JSON.parse(dekripsi(messageStr));
                        } catch (innerError) {
                            console.error("All parsing methods failed:", innerError);
                            throw new Error("Authentication data format is invalid");
                        }
                    }
                    ...
                    if (authData.name === "Sean" && authData.password === "bayar10rb") {
                        ...
                        // Send authentication success as plain string
                ...
                try {
                    // Simplify angle handling - parse as float directly
                    const messageStr = message.toString();
                    let currentAngle;
                    ...
                    try {
                        ...
                        console.error("Direct float parse failed:", e);
                        try {
                            currentAngle = parseFloat(dekripsi(messageStr));
                        } catch (innerError) {
                            console.error("Decryption failed:", innerError);
                    ...
                    if (!isNaN(currentAngle)) {
                        ...
                        // Broadcast position to all web clients as plain string
