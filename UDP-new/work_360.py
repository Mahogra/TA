...
class MotorController:
    ...
    async def handle_websocket_feedback(self):
        ...
        max_reconnect_delay = 10  # Maximum reconnect delay
        ...
        while self.running:
            try:
                ...
                async with websockets.connect(uri) as websocket:
                    # Authentication - Kirim data autentikasi sebagai JSON langsung
                    auth_data = {"name": "Sean", "password": "bayar10rb"}
                    await websocket.send(json.dumps(auth_data))
                    
                    # Terima respons autentikasi
                    ...
                    while self.running:
                        # Kirim data sudut langsung tanpa enkripsi
                        await websocket.send(str(self.anglecurr_total))
                        print(f"Position: {self.anglecurr_total * 180 / np.pi:.2f}°")
                        await asyncio.sleep(0.1)

            except websockets.exceptions.ConnectionClosed:
                print(f"WebSocket connection closed, reconnecting in {reconnect_delay} seconds...")
...
class UDPClientProtocol:
    ...
    def datagram_received(self, data, addr):
        try:
            # Simplifikasi proses - dekripsi pesan langsung
            message = data.decode()
            decrypted_message = dekripsi(message)
